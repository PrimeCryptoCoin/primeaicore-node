...
/**
 * Provides a friendly event driven API to primeaid in Node.js. Manages starting and
 * stopping primeaid as a child process for application support, as well as connecting
 * to multiple primeaid processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function primeaicoin(options) {
  if (!(this instanceof primeaicoin)) {
    return new primeaicoin(options);
  }
...
util.inherits(primeaicoin, Service);

primeaicoin.dependencies = [];

primeaicoin.DEFAULT_MAX_TXIDS = 1000;
primeaicoin.DEFAULT_MAX_HISTORY = 50;
primeaicoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
primeaicoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
primeaicoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
primeaicoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
primeaicoin.DEFAULT_SPAWN_STOP_TIME = 10000;
primeaicoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
primeaicoin.DEFAULT_REINDEX_INTERVAL = 10000;
primeaicoin.DEFAULT_START_RETRY_INTERVAL = 5000;
primeaicoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
primeaicoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
primeaicoin.DEFAULT_CONFIG_SETTINGS = {
  assetindex: 1,
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
...
primeaicoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || primeaicoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || primeaicoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || primeaicoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || primeaicoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || primeaicoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || primeaicoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || primeaicoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || primeaicoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || primeaicoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || primeaicoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

primeaicoin.prototype._initCaches = function() {
...
primeaicoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
primeaicoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
primeaicoin.prototype.getPublishEvents = function() {
  return [
    {
...
primeaicoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'primeaid/' + name, 'total:', this.subscriptions[name].length);
};

primeaicoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'primeaid/' + name, 'total:', this.subscriptions[name].length);
};

primeaicoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (primeaicore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'primeaid/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

primeaicoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'primeaid/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

primeaicoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
primeaicoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'primeaid/addresstxid', 'total:', _.size(this.subscriptions.address));
};

primeaicoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'primeaid/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
primeaicoin.prototype.unsubscribeAddressAll = function(emitter) {
...
primeaicoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = primeaicoin.DEFAULT_CONFIG_SETTINGS;
...
primeaicoin.prototype._parseprimeaicoinConf = function(configPath) {
  var options = {};
...
primeaicoin.prototype._expandRelativeDatadir = function() {
...
primeaicoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in primeaid config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in primeaid config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in primeaid config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = (spawnOptions['configPath']) ? path.resolve(spawnOptions.configPath) : path.resolve(spawnOptions.datadir, './primeai.conf');

  log.info('Using primeaicoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseprimeaicoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseprimeaicoinConf(networkConfigPath));
  }
...
primeaicoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of primeaicore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of primeaicore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of primeaicore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to primeaid from primeaicore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from primeaid. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from primeaid. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in primeai.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that primeaid is undergoing a reindex. ' +
             ...
             'of primeaicore-node services will start.');
    node._reindex = true;
  }
};

primeaicoin.prototype._resetCaches = function() {
...
primeaicoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
primeaicoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

primeaicoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('primeaicoin Daemon Ready');
          callback();
        });
      });
...
primeaicoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === primeaicore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

primeaicoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === primeaicore.Networks.testnet) {
    networkPath = 'testnet3/primeai.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/primeai.conf';
    }
  }
  return networkPath;
};

primeaicoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === primeaicore.Networks.testnet) {
...
primeaicoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('primeaid/hashblock', message.toString('hex'));
    }
  }

};

primeaicoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
primeaicoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('primeaicoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
primeaicoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
primeaicoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
primeaicoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('primeaid/addressbalance', {
                    address: address,
                    txid: txid,
...
primeaicoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('primeaid/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
primeaicoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = primeaicore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('primeaid/rawtransaction', message.toString('hex'));
    }
...
primeaicoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || primeaicoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

primeaicoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
primeaicoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
primeaicoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('primeaicoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || primeaicoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

primeaicoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
primeaicoin.prototype._stopSpawnedprimeaicoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/primeaid.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned primeaicoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean primeaicoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
primeaicoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedprimeaicoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting primeaicoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('primeaicoin process unexpectedly exited with code:', code);
        log.warn('Restarting primeaicoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('primeaicoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new primeaicoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn primeaid.'));
      }
...
primeaicoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to primeaid.'));
    }
 ...
 */
primeaicoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('primeaicoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
primeaicoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, primeaicore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
primeaicoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = primeaicore.Script.fromAddress(delta.address);
    return {
...
primeaicoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
primeaicoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
primeaicoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in primeaicore is most recent to less recent
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
primeaicoin.prototype._getConfirmationsDetail = function(transaction) {
...
primeaicoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
primeaicoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
primeaicoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A primeaicoin transaction id
 * @param {Function} callback
 */
primeaicoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
primeaicoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof primeaicore.Address) {
...
primeaicoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
primeaicoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
primeaicoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getRawBlock = function(blockArg, callback) {
...
primeaicoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a primeaicore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
primeaicoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = primeaicore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
primeaicoin.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
primeaicoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a primeaicore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
primeaicoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.getInfo = function(callback) {
  var self = this;
...
primeaicoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('primeaid spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('primeaid process did not exit'));
      }
...
primeaicoin.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
primeaicoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
primeaicoin.prototype.getMiningInfo = function(callback) {
    var self = this;
...
primeaicoin.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
primeaicoin.prototype.getPeerInfo = function(callback) {
    var self = this;
