...
/**
 * Will remove a service from primeaicore-node.json
...
/**
 * Will remove the Node.js service and from the primeaicore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The primeaicore-node configuration directory
...
function remove(options, done) {
  ...
  var primeaicoreConfigPath = path.resolve(configPath, 'primeaicore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(primeaicoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a primeaicore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to primeaicore-node.json
        removeConfig(primeaicoreConfigPath, service, next);
      });
    }, done
  );
}
