...
var primeaicoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for primeaid is necessary for this upgrade with the "reindex=1" primeai.conf option. \n' +
                 'There are changes necessary in both primeai.conf and primeaicore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/PrimeCryptoCoin/primeaicore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed primeaicore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './primeaicore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in primeaicore-node services directory
    ...
    if (servicePackage.primeaicoreNode) {
      serviceModule = service.name + '/' + servicePackage.primeaicoreNode;
    }
 ...
 * @param {Node} node - The primeaicore Node instance
 */
