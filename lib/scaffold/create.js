...
var BASE_PACKAGE = {
  description: 'A full primeaicoin node build with primeaicore',
  repository: 'https://github.com/PrimeCryptoCoin/primeaicore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
...
/**
 * Will create a directory and primeai.conf file for primeaicoin.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createprimeaicoinDirectory(datadir, done) {
...
/**
 * Will create a base primeaicore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The primeaicoin database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/primeaicore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a primeaicore Node directory, configuration file,
 * primeaicoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the primeaicore node configuration directory
 * @param {String} options.datadir - The path to the primeaicoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the primeaicore-node directory and configuration
    ...
    function(next) {
      // Setup the primeaicoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createprimeaicoinDirectory(absDataDir, next);
      } else {
        next();
      }
    },
